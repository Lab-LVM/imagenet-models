...
class FasterViT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_map:
            channels = [dim * 1, dim * 2, dim * 4, dim * 8, dim * 8]
            self.norm = nn.Identity()
            self.head = MAPHead(
                # multi-scale
                multi_scale_level=3, channels=channels, last_dim=384,
                # multi-token
                n_tokens=3, n_groups=5, self_distill_token=True,
                # gram
                non_linearity=nn.GELU, gram=True, concat_blk=None, gram_blk=nn.Identity,
                bp_dim=384, bp_groups=1, gram_group=32, gram_dim=384,
                # class attention
                num_heads=32, ca_dim=384, mlp_ratio=4, mlp_groups=2, interactive=True,
                # FC layer
                head_fn=nn.Linear, fc_drop=drop_rate, num_classes=num_classes,
            )
        else:
...
@register_model
def map_faster_vit_3_224(pretrained=False, **kwargs):
    depths = kwargs.pop("depths", [3, 3, 12, 5])
    num_heads = kwargs.pop("num_heads", [2, 4, 8, 16])
    window_size = kwargs.pop("window_size", [7, 7, 7, 7])
    ct_size = kwargs.pop("ct_size", 2)
...
if __name__ == '__main__':
    ...
    model = create_model('faster_vit_3_224_map')
    y = model(x)
    print(y.shape)
